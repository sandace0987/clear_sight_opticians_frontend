...
export const Route = createFileRoute("/brands_/$brand")({
  head: ({ params }) => {
    ...
    const title = b ? `${b.name} Eyewear in Hyderabad | Clear Sight Opticians` : "Designer Eyewear Brands | Clear Sight Opticians";
    const desc = b ? `Shop authentic ${b.name} prescription glasses and sunglasses at Clear Sight Opticians in Hyderabad. Explore models at KPHB, Nizampet, and Bowenpally.` : "Curated designer frames and prescription eyewear.";
    const kws = b ? `${b.name} Hyderabad, ${b.name} glasses, authentic ${b.name} frames, ${b.name} sunglasses India, designer eyewear Kukatpally` : "designer eyewear, optical shops Hyderabad";
    const url = `https://www.clearsightopticians.in/brands/${params.brand}`;
    return {
      meta: [
        { title },
